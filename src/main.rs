...
/// }
/// ```
async fn search_for_movies(client: &reqwest::Client, mongo_client: &Client) -> Result<(), Box<dyn std::error::Error>>{
...
/// }
/// ```
async fn search_for_existing_movie(mongo_client: &Client) -> Result<(), Box<dyn std::error::Error>> {
    let db = mongo_client.database("Sratim");
    let queue = db.collection::<Document>("Queue");
...
async fn delete_movie() -> Result<(), Box<dyn std::error::Error>> {
    Ok(())
}
...
/// }
/// ```
async fn empty_queue(mongo_client: &Client) -> Result<(), Box<dyn std::error::Error>> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        match choice {
            1 => search_for_movies(&client, &mongo_client).await?,
            2 => search_for_existing_movie(&mongo_client).await?,
            3 => delete_movie().await?,
            4 => empty_queue(&mongo_client).await?,
            _ => break,
        }
